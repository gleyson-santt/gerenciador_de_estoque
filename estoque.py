...
def adicionar_produto():
    ...
    try:
        ...
        conn.commit()
        atualizar_lista()
        entry_nome.delete(0, tk.END); entry_quantidade.delete(0, tk.END); entry_validade.delete(0, tk.END)
...
def atualizar_quantidade():
    ...
    if not sel: return messagebox.showwarning("Selecione", "Selecione um produto.")
    if not novo.isdigit(): return messagebox.showwarning("Valor inválido", "Quantidade deve ser número.")
    pid = tree.item(sel)["values"][0]
    cursor.execute("UPDATE produtos SET quantidade=? WHERE id=?", (int(novo), pid))
    conn.commit(); atualizar_lista()
...
def filtrar_por_mes():
    m = entry_mes.get().strip(); a = entry_ano.get().strip()
...
def editar_produto():
    ...
    edit = tk.Toplevel(root); edit.title("Editar Produto")
    edit.configure(bg="white"); edit.geometry("380x230"); edit.grab_set()

    ttk.Label(edit, text="Nome:").grid(row=0, column=0, padx=10, pady=8, sticky="w")
    e_nome = ttk.Entry(edit, width=30); e_nome.insert(0, n0); e_nome.grid(row=0, column=1, pady=8)

    ttk.Label(edit, text="Quantidade:").grid(row=1, column=0, padx=10, pady=8, sticky="w")
    e_q = ttk.Entry(edit, width=10); e_q.insert(0, q0); e_q.grid(row=1, column=1, pady=8, sticky="w")

    ttk.Label(edit, text="Validade (YYYY-MM-DD):").grid(row=2, column=0, padx=10, pady=8, sticky="w")
    e_v = ttk.Entry(edit, width=20); e_v.insert(0, v0); e_v.grid(row=2, column=1, pady=8, sticky="w")
    ...
    def salvar():
        ...
        if not nn or not nq or not nv:
            return messagebox.showwarning("Campos vazios", "Preencha todos os campos.")
        try:
            nq = int(nq); datetime.strptime(nv, "%Y-%m-%d")
            cursor.execute("UPDATE produtos SET nome=?,quantidade=?,validade=? WHERE id=?",
                           (nn, nq, nv, pid))
            conn.commit(); atualizar_lista()
            messagebox.showinfo("Sucesso", "Produto atualizado."); edit.destroy()
        except ValueError:
    ...
    def excluir():
        if messagebox.askyesno("Confirmar", "Excluir este produto?"):
            ...
            conn.commit(); atualizar_lista()
            messagebox.showinfo("Removido", "Produto excluído."); edit.destroy()

    fb = ttk.Frame(edit); fb.grid(row=3, column=0, columnspan=2, pady=15)
    ttk.Button(fb, text="Salvar", command=salvar).grid(row=0, column=0, padx=8)
    rb = ttk.Button(fb, text="Excluir", command=excluir)
    rb.grid(row=0, column=1, padx=8); rb.configure(style="Danger.TButton")
...
# Frames
f_top = ttk.Frame(root, padding=10); f_top.pack(pady=10)
f_table = ttk.Frame(root, padding=10); f_table.pack(padx=20, pady=10, fill="both", expand=True)
f_bot = ttk.Frame(root, padding=10); f_bot.pack(pady=10)

# Campos inserção
ttk.Label(f_top, text="Produto:").grid(row=0, column=0, padx=5, pady=5)
entry_nome = ttk.Entry(f_top, width=30); entry_nome.grid(row=0, column=1, padx=5)
ttk.Label(f_top, text="Quantidade:").grid(row=0, column=2, padx=5)
entry_quantidade = ttk.Entry(f_top, width=10); entry_quantidade.grid(row=0, column=3, padx=5)
ttk.Label(f_top, text="Validade:").grid(row=0, column=4, padx=5)
entry_validade = ttk.Entry(f_top, width=15); entry_validade.grid(row=0, column=5, padx=5)
...
entry_nova_qtde = ttk.Entry(f_bot, width=10); entry_nova_qtde.grid(row=0, column=1)
...
entry_mes = ttk.Entry(f_bot, width=5); entry_mes.grid(row=1, column=1)
ttk.Label(f_bot, text="Ano:").grid(row=1, column=2, padx=5)
entry_ano = ttk.Entry(f_bot, width=7); entry_ano.grid(row=1, column=3)
